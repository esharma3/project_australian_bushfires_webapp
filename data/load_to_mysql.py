...
# 2019
GLOBAL_FIRES_2019 = "g_fires2019" 
engine.execute(f"DROP TABLE IF EXISTS {GLOBAL_FIRES_2019}")

schema = {'latitude': sqlalchemy.types.Float(precision=7, asdecimal=True),
          'longitude': sqlalchemy.types.Float(precision=7, asdecimal=True),
          'acq_date': sqlalchemy.types.String(length=30),
          'acq_time': sqlalchemy.types.String(length=30),
          'instrument': sqlalchemy.types.String(length=30),
          'type': sqlalchemy.types.INTEGER()
         }

df = pd.read_csv("global_fires_data/fd_2019.csv").to_sql(
    name = GLOBAL_FIRES_2019,
    con = engine,
    dtype = schema)

engine.execute(f"ALTER TABLE {GLOBAL_FIRES_2019} ADD PRIMARY KEY (`index`)")

# 2018
GLOBAL_FIRES_2018 = "g_fires2018" 
engine.execute(f"DROP TABLE IF EXISTS {GLOBAL_FIRES_2018}")
...
df = pd.read_csv("global_fires_data/fd_2018.csv").to_sql(
    name = GLOBAL_FIRES_2018,
    con = engine,
    dtype = schema)

engine.execute(f"ALTER TABLE {GLOBAL_FIRES_2018} ADD PRIMARY KEY (`index`)")

# 2017
GLOBAL_FIRES_2017 = "g_fires2017" 
engine.execute(f"DROP TABLE IF EXISTS {GLOBAL_FIRES_2017}")
...
df = pd.read_csv("global_fires_data/fd_2017.csv").to_sql(
    name = GLOBAL_FIRES_2017,
    con = engine,
    dtype = schema)

engine.execute(f"ALTER TABLE {GLOBAL_FIRES_2017} ADD PRIMARY KEY (`index`)")
